...
// GET - List hunt sessions for a party
export async function GET(
  ...
  try {
    ...
    const sessions = await prisma.huntSession.findMany({
      where: {
        partyId: id,
        ...(dateFilter ? { huntDate: { gte: dateFilter } } : {}),
      },
      orderBy: { huntDate: "desc" },
    });

    return NextResponse.json(serializeBigInt(sessions));
  } catch (error) {
    console.error("Error fetching hunt sessions:", error);
    return NextResponse.json(
      { error: "Failed to fetch hunt sessions" },
      { status: 500 },
    );
  }
}
